...
def gradcheck_naive(f, x, gradient_text=""):
    ...
    while not it.finished:
        ...
        x[ix] += h
        random.setstate(rndstate)
        f_plus_h, f_grad_plus = f(x)
        x[ix] -= (2 * h)
        random.setstate(rndstate)
        f_minus_h, f_grad_minus = f(x)
        x[ix] += h 
        numgrad = (f_plus_h - f_minus_h) / (2*h)  # centered difference
        ### END YOUR CODE
...
def your_gradcheck_test():
    ...
    sin = lambda x: (np.sum(np.sin(x)), np.cos(x))  # derivative of sin is cos
    exp = lambda x: (np.sum(np.exp(x)), np.exp(x)) 

    print("Running sanity checks...")
    for f in [sin, exp]:
        gradcheck_naive(f, np.array(123.456))       # scalar test
        gradcheck_naive(f, np.random.randn(3,))     # 1-D test
        gradcheck_naive(f, np.random.randn(4, 5))   # 2-D test
        print()
    ### END YOUR CODE
