...
# Save the device
device = torch.cuda.current_device() if torch.cuda.is_available() else 'mps' if torch.backends.mps.is_available() else 'cpu'
...
if args.function == 'pretrain':
    ...
    # pretrain_dataset is already given at line 51-52 taking from args.pretrain_corpus_path
    # 1. Pretrain the model on this corpus
    ...
    # 1. If args.reading_params_path is specified, load these parameter into the model
    ...
    if args.eval_corpus_path is not None:
        with open(args.eval_corpus_path, "r", encoding='utf-8') as f:
            eval_text = f.read()
        eval_dataset = dataset.NameDataset(pretrain_dataset, eval_text)
    else:
        eval_dataset = None
    ...
    tconf = trainer.TrainerConfig(max_epochs=max_epochs, batch_size=256, learning_rate=args.finetune_lr,
                      lr_decay=True, warmup_tokens=512*20, final_tokens=200*len(pretrain_dataset)*block_size,
                      num_workers=0, writer=writer)
    trainer = trainer.Trainer(model, finetune_dataset, eval_dataset, tconf)
    trainer.train()
