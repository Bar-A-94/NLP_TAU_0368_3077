...
class NameDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        x = x[:-1]
        x = torch.tensor([self.stoi[c] for c in x], dtype=torch.long, device=device)
        y = torch.tensor([self.stoi[c] for c in y], dtype=torch.long, device=device)
        return x, y
...
class CharCorruptionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # The length of [masked_content] should be random, and 1/4 the length of the
        #     truncated document on average.
        masked_content_length = random.randint(1, len(element)//2) # randint generates values uniformly
        start_mask_index = random.randint(0, len(element) - masked_content_length)
        prefix = element[:start_mask_index]
        masked_content = element[start_mask_index:start_mask_index + masked_content_length]
        suffix = element[start_mask_index + masked_content_length:]
